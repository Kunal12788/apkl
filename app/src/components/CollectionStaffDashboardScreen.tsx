          ...
          <div className="luxury-card overflow-hidden bg-white border border-outline-variant/10">
            {dynamicRecentTasks.length > 0 ? (
              dynamicRecentTasks.slice(0, 5).map((item, idx) => {
                ...
                return (
                  <div key={idx} className={`p-5 flex items-center justify-between group transition-colors border-b last:border-0 border-outline-variant/10 ${isCash ? 'cash-watermark-row hover:bg-surface-container-lowest bg-white' : 'hover:bg-surface-container-lowest bg-white'}`}>
                    <div className="flex items-center gap-4">
